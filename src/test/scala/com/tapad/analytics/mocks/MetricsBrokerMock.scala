...
object MetricsBrokerMock extends MockHelpers[MetricsBrokerMock] {
  ...
  case class ReportAll(in: Chunk[MetricEvent]) extends Invocation[ReportAll, Chunk[MetricEvent]]
  ...
  val compose: ULayer[MetricsBrokerMock] =
    ZLayer {
      for {
        ...
        override def reportAll(batch: Chunk[MetricEvent]): UIO[Unit] = invocationRef.update(_ :+ ReportAll(batch))
